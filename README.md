...
Для работы с данной сборкой в новом проекте, скачать архив с репозитория,разорхивировать, открыть в vscode.
...
В _settings.scss в body прописать основной шрифт.

Также не забудьте прописать эти же шрифты в `<link preload>` в html.Для ускорения сайта , быстрой загрузки шрифтов(выдает ошибку в консоли)
...
## Начало работы ==============================
1. Скачать и установить шрифты(инструкция выше).
2. Настройки контейнера 
3. Иконочный шрифт если нужно или спрайт 
4. Свои переменные scss и если нужно миксины 
